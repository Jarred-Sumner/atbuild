...
In this templating language, there are four keywords. All other text is assumed to be part of the parent scope. From the language's perspective, if the text isn't `@build`, `@run`, `@()`, `@}`, or `@export function`, like Jon Snow, it knows nothing.

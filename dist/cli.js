...
try {
  ...
  console.error(`Please pass a valid input file. Received ${path.join(__dirname, cli.flags.input[0])}`);
  process.exit(1);
}
...
if (cli.flags.input[1]) {
  ...
  try {
    ...
    console.error(`Output directory doesn't exist. Received ${path.join(__dirname, cli.flags.input[0])}`);
    process.exit(1);
  }
